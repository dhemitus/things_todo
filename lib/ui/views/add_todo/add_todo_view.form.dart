...
final Map<String, TextEditingController> _AddTodoViewTextEditingControllers =
    {};

final Map<String, FocusNode> _AddTodoViewFocusNodes = {};

final Map<String, String? Function(String?)?> _AddTodoViewTextValidations = {
  TitleValueKey: null,
...
mixin $AddTodoView {
  ...
  TextEditingController _getFormTextEditingController(
    ...
    if (_AddTodoViewTextEditingControllers.containsKey(key)) {
      return _AddTodoViewTextEditingControllers[key]!;
    }

    _AddTodoViewTextEditingControllers[key] =
        TextEditingController(text: initialValue);
    return _AddTodoViewTextEditingControllers[key]!;
  }

  FocusNode _getFormFocusNode(String key) {
    if (_AddTodoViewFocusNodes.containsKey(key)) {
      return _AddTodoViewFocusNodes[key]!;
    }
    _AddTodoViewFocusNodes[key] = FocusNode();
    return _AddTodoViewFocusNodes[key]!;
  }
  ...
  void disposeForm() {
    ...
    for (var controller in _AddTodoViewTextEditingControllers.values) {
      controller.dispose();
    }
    for (var focusNode in _AddTodoViewFocusNodes.values) {
      focusNode.dispose();
    }

    _AddTodoViewTextEditingControllers.clear();
    _AddTodoViewFocusNodes.clear();
  }
}
...
extension ValueProperties on FormStateHelper {
  ...
  set titleValue(String? value) {
    ...
    if (_AddTodoViewTextEditingControllers.containsKey(TitleValueKey)) {
      _AddTodoViewTextEditingControllers[TitleValueKey]?.text = value ?? '';
    }
  }
  ...
  set descriptionValue(String? value) {
    ...
    if (_AddTodoViewTextEditingControllers.containsKey(DescriptionValueKey)) {
      _AddTodoViewTextEditingControllers[DescriptionValueKey]?.text =
          value ?? '';
    }
  }
...
String? getValidationMessage(String key) {
  final validatorForKey = _AddTodoViewTextValidations[key];
  if (validatorForKey == null) return null;

  String? validationMessageForKey = validatorForKey(
    _AddTodoViewTextEditingControllers[key]!.text,
  );
